...
- **오류 복구**: 진행 중 예기치 못한 오류가 나면 안내 창을 띄우고, 마지막으로 저장된 지점에서 이어가거나 타이틀로 돌아갈 수 있습니다. 같은 전투에서 오류가 반복되면 **전투를 처음부터 다시**하거나 **보상 없이 건너뛸** 수 있고, 버그 제보용으로 오류와 세이브 내용을 복사할 수 있습니다.
...
5. 적 턴 도중 일부러 오류를 내서 복구되는지: 저장 지점에서 이어가기, 반복될 때 처음부터 다시·건너뛰기·오류 정보 복사
6. 4개 클래스 자동 플레이(탐욕 봇)로 전체 흐름을 돌리며 런타임 에러와 진행 멈춤 감지

`SMOKE_BUDGET_S`(클래스당 시간 예산, 초)와 `SMOKE_CLASSES`(예: `gambler,gambler,gambler`)로 조정할 수 있습니다. 실행할 때마다 엔진 난수 시드를 출력하므로, 실패하면 `SMOKE_SEED=<시드>`로 같은 진행을 재현할 수 있습니다. 두 테스트 모두 `main` 푸시와 PR마다 GitHub Actions에서 실행됩니다.
