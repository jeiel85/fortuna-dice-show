...
- **오류 복구**: 진행 중 예기치 못한 오류가 나면 안내 창을 띄우고, 마지막으로 저장된 지점에서 이어가거나 타이틀로 돌아갈 수 있습니다.
- **세이브 버전**: 저장 형식이 바뀌면 `SAVE_V`를 올리고 `MIGRATIONS`에 이전 버전을 변환하는 단계를 추가합니다. 과거 버전의 실제 세이브(`tests/fixtures/save-v*.json`)가 항상 최신 형식으로 불러와지는지 테스트합니다.
...
5. 적 턴 도중 일부러 오류를 내서 안내 창 → 저장 지점에서 이어가기로 복구되는지
