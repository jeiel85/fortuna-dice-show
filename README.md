...
`index.html`에서 엔진 블록만 떼어 Node `vm`에서 시드 고정 난수로 실행합니다. 피해 계산(방어·관통·회피·가시·힘·약화), 적 예고에 거는 상태이상, 적 AI 배치 순서, 주사위 배치(여러 칸·같은 눈·카운트다운·도구), 턴 시작 처리, 적 턴 중간 저장 후 이어가기, 스킬, 보상·레벨 업, 맵 생성 규칙(시드 60개), 상점·장비 관리·보물 상자·사과나무·대장간·룰렛·주사위 대결·여신의 거래 규칙, 세이브 검증을 확인합니다. 엔진 블록이 브라우저 API를 참조하지 않는지, 엔진이 내보내는 이벤트와 UI가 처리하는 이벤트가 일치하는지도 검사합니다.
...
4. 실제 입력: 마우스 드래그 앤 드롭, 클릭 선택, 키보드(주사위 버튼 → 장비 카드), 접근성 트리의 역할·이름
5. 4개 클래스 자동 플레이(탐욕 봇)로 전체 흐름을 돌리며 런타임 에러와 진행 멈춤 감지
